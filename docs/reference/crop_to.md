...
- X:
  ...
  samples data in any frame and unit.
...
- tcues:
  ...
  A two-element vector containing the start and end time cues in seconds
...
Cropped data in the same format as X, unless X is irregularly sampled
and NOT a sensor list. In that case, the function returns a list with 2
elements:

- **X:** A sensor list, vector or matrix containing the cropped data
