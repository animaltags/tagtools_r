...
#' @note This function is based on the Matlab fir_nodelay() function in the matlab animaltags tool kit, and currently uses \code{fir1()} from package gsignal. Whether we use \code{fir1} from the signal or the gsignal R package, the filter coefficients returned are *not* exactly identical to the ones returned by Matlab's fir1() function and we don't have a simple remedy for this.
#' @examples 
...
fir_nodelay <- function(x, n, fc, qual = "low", return_coefs = FALSE) {
  ...
  # generate fir filter
  # ============================================================
  h <- gsignal::fir1(n = n, w = fc, type = qual)
