...
#' @param tfm The target field magnitude e.g., 9.81 for accelerometer data using m/s^2 as the unit.
...
spherical_cal <- function(X, tfm = NULL, method = "offset_scaling") {
  if (! method %in% c("offset_scaling", "cross", "gain")){
    stop("Unrecognized method. Options are: 'offset_scaling' or 'cross' or 'gain'")
  }
  
  # remove any rows in X with NaNs
  if (anyNA(X)){
    X <- stats::na.omit(X)
    warning("Missing values in X have been omitted.")
  }
  ...
  offs <- H[1:3]
  X <- X + matrix(offs, nrow = nrow(X), ncol = 3, byrow = TRUE)
  # now try up to three calibration scenarios using simplex search
  
  C <- matrix(0, nv3, 3)
  
  # offset only cal
  C[1:nv1, 1] <- stats::optim(matrix(0, nv1, 1), (function(c) ccost(as.matrix(c), X)))$par 
  
  # add on gain cal
  if (identical(method, "gain") | identical(method, "cross")) {
    C[1:nv2, 2] <- stats::optim(C[1:nv2, 1], (function(c) ccost(as.matrix(c), X)))$par 
  }
  
  # add on cross cal
  if (identical(method, "cross")) {
    C[, 3] <- stats::optim(C[, 2], (function(c) ccost(as.matrix(c), X)))$par 
  }
  ...
  G <- list(residual = sprintf("%2.1f ", 100 * stats::sd(nn) / mean(nn)),
            axial_balance = sprintf("%2.1f", 100 / pracma::cond(R)),
            poly = cbind((1 + C[, 2]) * sf, ((offs * (1 + C[, 2])) + C[, 1]) * sf),
            cross = 0.5 * rbind(c(2, C[1, 3], C[3, 3]), c(C[1, 3], 2, C[2, 3]), c(C[3, 3], C[2, 3], 2)))
...
appcal <- function(X, C) {
  ...
  Y <- X %*% diag(1 + C[, 2]) + matrix(C[,1], nrow = nrow(X), ncol = 3, byrow = TRUE)
