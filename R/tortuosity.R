...
tortuosity <- function(track, sampling_rate, intvl) {
  ...
  # We are not sure what column 2 is measuring so not returning for now
  # tortuosity <- cbind(t, matrix(0, nrow(tortuosity), 1))
  # tortuosity[, 2] <- t(sqrt(colMeans((N - matrix(colMeans(N), nrow = nrow(N), ncol = ncol(N), byrow = TRUE))^2 +
  #                           (E - matrix(colMeans(E), nrow = nrow(E), ncol = ncol(E), byrow = TRUE))^2)))
    return(tortuosity)
}
