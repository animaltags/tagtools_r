...
sm_fnames <- function(sm_dir,
  ...
  if (length(sm_xml) > 0){
    ...
    sm_file_meta <- data.frame(file_name = gsub(sm_xml, pattern = ".xml", replacement = "", fixed = TRUE),
                            sm_dir = sm_dir,
                            recn = 0,
                            device_serial = 0,
                            device_id = ''
    ...
    for (f in c(1:length(sm_xml))){
      ...
      xml_info <- sm_xml_devid(xml_file)
      sm_file_meta[f, "device_serial"] <- xml_info$device_serial
      sm_file_meta[f, "device_id"] <- xml_info$device_id
      # the record number is the last three characters of the extension-less file name
      sm_file_meta[f, "recn"] <- as.numeric(stringr::str_sub(sm_file_meta[f, "file_name"], start = -3, end = -1))
    }
  }else{
