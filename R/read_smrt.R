...
#' @param nc_dir String containing the name (including full or relative path) of the directory where the output nc file should be stored. Defaults to \code{data_dir}.
...
#' @param quiet Whether or not to print progress updates to screen. Default: TRUE to give no console output. (Could be desirable to use \code{quiet = FALSE} track progress of processing large datasets, which can be a bit slow!)
...
read_smrt <- function(data_dir, 
                      ...
                      quiet = TRUE) {
  if (!quiet){
    message("Checking inputs and gathering metadata...")
  }
  
  # Input checking
  if (missing(depid) | missing(data_dir)){
    stop("Inputs data_dir and depid are required to read SMRT tag data.")
  }
  ...
  if (read_gps & !is.null(gps_file)){
    ...
    if (!quiet){
      timing <- proc.time() - start_op
      message(paste0("GPS data read in ", round(timing[3], digits = 2), " seconds."))
    }
  }
  ...
  if (read_archive & !is.null(archive_file)){
    if (!quiet){
      start_op <- proc.time()
      message("Reading sensor data from WC board Archive file...")
    }
    ...
    if (!quiet){
      timing <- proc.time() - start_op
      message(paste0("Archive data read in ", round(timing[3], digits = 2), " seconds."))
    }
  }
  ...
  if (!quiet){
    start_op <- proc.time()
    message("Processing acoustic metadata...")
  }
  ...
  if (!quiet){
    timing <- proc.time() - start_op
    message(paste0("Acoustic metadata processed in ", round(timing[3], digits = 2), " seconds."))
  }
  ...
  if (read_sm_swv | read_sm_csv){
    ...
    if (!quiet){
      timing <- proc.time() - start_op
      message(paste0("SM data read in ", round(timing[3], digits = 2), " seconds."))
    }
  }
  ...
  if (read_sm_swv | read_sm_csv){
    if (!quiet){
      start_op <- proc.time()
      message("Saving SM data to netCDF...")
    }
    ...
    if (!quiet){
      timing <- proc.time() - start_op
      message(paste0("SM data saved to .nc in ", round(timing[3], digits = 2), " seconds."))
    }
  }
  ...
  if (read_gps & exists("gps_sensor_list", mode = "list")){
    if (!quiet){
      start_op <- proc.time()
      message("Saving GPS data to netCDF...")
    }
    save_sensor_lists(nc_file = nc_file, sensor_list_list = gps_sensor_list)
    if (!quiet){
      timing <- proc.time() - start_op
      message(paste0("GPS data saved to .nc in ", round(timing[3], digits = 2), " seconds."))
    }
  }
  ...
  if (read_archive & exists("archive_sensor_list", mode = "list")){
    if (!quiet){
      start_op <- proc.time()
      message("Saving WC board Archive data to netCDF...")
    }
    save_sensor_lists(nc_file = nc_file, sensor_list_list = archive_sensor_list)
    if (!quiet){
      timing <- proc.time() - start_op
      message(paste0("Archive data saved to .nc in ", round(timing[3], digits = 2), " seconds."))
    }
  }
